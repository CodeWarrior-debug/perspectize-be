...
files_modified:
  ...
  - perspectize-fe/src/lib/components/Navigation.svelte
  - perspectize-fe/src/routes/+page.svelte
  - perspectize-fe/src/routes/add-video/+page.svelte
...
must_haves:
  truths:
    - "User can navigate between Activity and Add Video pages via header"
    - "Navigation visually indicates current page"
  ...
  artifacts:
    - path: "perspectize-fe/src/lib/components/Navigation.svelte"
      provides: "Navigation component with links"
      contains: "href"
    - path: "perspectize-fe/src/routes/add-video/+page.svelte"
      provides: "Add Video page"
      min_lines: 10
...
<objective>
Implement header navigation between Activity and Add Video pages, and validate AG Grid Svelte 5 wrapper functionality with a comprehensive feature test.

Purpose: Complete the navigation skeleton so users can move between pages, and derisk the AG Grid community wrapper by validating all required features (sorting, filtering, pagination, column resize, row selection) before committing to it for Phase 2.

Output: Working navigation in header, Add Video page stub, and AG Grid validation report documenting which features work/fail.
</objective>
...
<task type="auto">
  <name>Task 1: Create Navigation component and Add Video page</name>
  <files>
    perspectize-fe/src/lib/components/Navigation.svelte
    perspectize-fe/src/lib/components/Header.svelte
    perspectize-fe/src/routes/add-video/+page.svelte
  ...
  <action>
    1. Create Navigation.svelte in src/lib/components/:
       ```svelte
       <script lang="ts">
       ...
       Key features:
       - Uses SvelteKit's page store to determine current route
       - Active page uses 'default' variant (navy), inactive uses 'ghost'
       - Uses shadcn Button for consistent styling

    2. Update Header.svelte to include Navigation:
       ```svelte
       <script lang="ts">
       ...
       Changes:
       - Added sticky positioning so header stays visible on scroll
       - Added z-50 for proper stacking
       - Logo is now a link to home
       - Removed slot, using Navigation component directly

    3. Create Add Video page stub at src/routes/add-video/+page.svelte:
       ```svelte
       <script lang="ts">
       ...
       <PageWrapper>
         <h1 class="text-3xl font-bold mb-4">Add Video</h1>
         <p class="text-muted-foreground mb-8">
           Paste a YouTube URL to add a video to Perspectize.
         </p>

         <div class="p-8 border border-dashed border-border rounded-lg text-center text-muted-foreground">
           Video URL input will be implemented in Phase 3
         </div>
       </PageWrapper>
       ```
  </action>
  <verify>
    ...
    - Click "Add Video" in header - navigates to /add-video
    - Click "Activity" in header - navigates to /
    - Current page button is navy (default variant)
    - Other page button is ghost variant
  </verify>
  <done>
    - Navigation.svelte created with Activity and Add Video links
    - Header.svelte updated to include Navigation
    - Header is sticky with proper z-index
    - Add Video page exists at /add-video
    - Navigation shows current page with navy button
...
<task type="auto">
  ...
  <action>
    ...
    4. Update +page.svelte to include AG Grid test (replace previous test content):
       ```svelte
       <script lang="ts">
...
<task type="auto">
  ...
  <done>
    ...
    - Recommendation for Phase 2 included
    - Any issues noted
  </done>
</task>
...
1. **Navigation works**: Can navigate between Activity (/) and Add Video (/add-video)
2. **Current page indicated**: Active nav item uses navy (default) button variant
3. **AG Grid renders**: Test table visible on Activity page
4. **Features validated**: All 6 AG Grid features tested and documented
5. **Validation documented**: AG_GRID_VALIDATION.md contains results
6. **Decision recorded**: Recommendation for Phase 2 (proceed vs fallback)
</verification>

<success_criteria>
- Navigation.svelte created with Activity and Add Video links
- Header includes Navigation, is sticky
- Add Video page exists at /add-video route
