...
files_modified:
  ...
  - perspective-fe/tests/helpers/render.ts
  ...
  - perspectize-fe/tests/unit/layout-config.test.ts
...
must_haves:
  ...
  artifacts:
    - path: "perspectize-fe/tests/unit/utils.test.ts"
      provides: "Unit tests for cn() utility"
      contains: "cn("
      min_lines: 20
    - path: "perspectize-fe/tests/unit/queries-client.test.ts"
      provides: "Unit tests for GraphQL client setup"
      contains: "graphqlClient"
      min_lines: 15
    - path: "perspectize-fe/tests/unit/queries-content.test.ts"
    ...
    - path: "perspectize-fe/tests/components/Header.test.ts"
      ...
      contains: "Header"
      min_lines: 20
    - path: "perspectize-fe/tests/components/PageWrapper.test.ts"
      provides: "Component tests for PageWrapper.svelte"
      contains: "PageWrapper"
      min_lines: 20
    - path: "perspectize-fe/tests/helpers/render.ts"
    ...
    - path: "perspectize-fe/vite.config.ts"
      ...
      contains: "thresholds"
  key_links:
    - from: "perspectize-fe/tests/components/Header.test.ts"
      to: "perspectize-fe/src/lib/components/Header.svelte"
      via: "import and render"
      pattern: "import.*Header"
    - from: "perspectize-fe/tests/components/PageWrapper.test.ts"
      to: "perspectize-fe/src/lib/components/PageWrapper.svelte"
      via: "import and render"
      pattern: "import.*PageWrapper"
    - from: "perspectize-fe/tests/unit/utils.test.ts"
      to: "perspectize-fe/src/lib/utils.ts"
...
<task type="auto">
  ...
  <action>
    ...
    **Step 7: Create `tests/unit/layout-config.test.ts` for the layout.ts config.**

    ```typescript
    import { describe, it, expect } from 'vitest';
    import { prerender } from '../../src/routes/+layout';

    describe('+layout.ts configuration', () => {
      it('exports prerender as true for static site generation', () => {
        expect(prerender).toBe(true);
      });
    });
    ```

    NOTE: Use a relative import path since `$app` paths are for SvelteKit modules only. The +layout.ts file is a plain TypeScript export.
  </action>
  <verify>
    Run all unit tests:
    ```bash
    cd perspectize-fe && pnpm test:run -- tests/unit/
    ```
    All tests pass with zero failures.

    Verify placeholder test is gone:
    ```bash
    grep -r "1 + 1" perspectize-fe/tests/
    ```
    No results (placeholder removed).

    Verify test helper exists:
    ```bash
    test -f perspectize-fe/tests/helpers/render.ts && echo "exists"
    ```
  </verify>
  <done>
    ...
    - tests/unit/layout-config.test.ts verifies prerender=true
...
<task type="auto">
  ...
  <action>
    **Step 1: Update tests/setup.ts to add mocks needed for component tests.**
    ...
    NOTE: The `$app/stores` mock uses `await import('svelte/store')` inside the factory — Vitest supports top-level await in mock factories. If this causes issues, use a simpler object mock instead. The favicon mock prevents import errors when testing +layout.svelte.
    ...
    **Step 3: Create `tests/components/PageWrapper.test.ts`.**
    ...
    describe('PageWrapper component', () => {
      it('renders without errors', () => {
        // PageWrapper requires children prop (Svelte 5 snippet).
        // Render with minimal setup — @testing-library/svelte handles
        // Svelte 5 components that use $props() and {@render children()}.
      ...
      it('applies custom className when provided', () => {
        const { container } = render(PageWrapper, { props: { class: 'custom-class' } });
        const main = container.querySelector('main');
      ...
      it('applies empty className by default', () => {
        const { container } = render(PageWrapper);
        const main = container.querySelector('main');
        // Default className is '' which results in a trailing space in the class string
        // The main element should still have base classes
        expect(main?.className).toContain('px-4');
      });
    });
    ```

    NOTE: PageWrapper uses `{@render children()}` which is Svelte 5 syntax. When rendered without children via @testing-library/svelte, it will render the main element with no content. This is fine for structural tests. If @testing-library/svelte v5.3.1 throws on missing children, wrap the render call in try/catch and assert the main element was rendered before the error. Alternatively, if children is required and causes a render error, test only with `render(PageWrapper)` and check the container output.
    ...
    **Step 4: Configure coverage thresholds in vite.config.ts.**
    ...
    coverage: {
      ...
      exclude: [
        ...
        // Exclude route pages from coverage (tested via integration/E2E)
        'src/routes/**',
      ],
      thresholds: {
        lines: 80,
    ...
    - `src/routes/**` — route pages (+page.svelte, +layout.svelte) are integration-level; they contain mostly markup and framework wiring. Testing them with unit tests creates brittle mocks of QueryClientProvider, Toaster, etc. They will be tested via E2E in Phase 5.
    ...
    **Step 5: Run full test suite with coverage and verify.**
    ...
    ```bash
    cd perspectize-fe && pnpm test:coverage
    ```
  ...
  <verify>
    Run full test suite:
    ```bash
    cd perspectize-fe && pnpm test:run
    ```
    ...
    ```bash
    grep -q "thresholds" perspectize-fe/vite.config.ts && echo "thresholds configured"
    ```

    Verify test organization:
    ```bash
    ls perspectize-fe/tests/unit/ perspectize-fe/tests/components/ perspectize-fe/tests/helpers/
    ```
    Expected: unit/ has utils, queries-client, queries-content, shadcn-barrel, layout-config test files. components/ has Header, PageWrapper test files. helpers/ has render.ts.
  </verify>
  <done>
    - tests/components/Header.test.ts covers rendering, brand text, structure, responsive classes, nav element
    - tests/components/PageWrapper.test.ts covers rendering, main element, responsive classes, custom className prop
    ...
    - tests/setup.ts updated with $app/stores mock and favicon mock for component tests
...
- PageWrapper component renders main element, responsive classes, custom className prop
- prerender=true configuration verified
