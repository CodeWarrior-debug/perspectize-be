...
must_haves:
  ...
  artifacts:
    ...
    - path: "frontend/src/lib/queries/content.ts"
      ...
      min_lines: 20
    - path: "frontend/src/lib/stores/userSelection.svelte.ts"
    ...
    - path: "frontend/tests/unit/stores-userSelection.test.ts"
      ...
      min_lines: 20
  key_links:
    - from: "frontend/src/lib/queries/users.ts"
      to: "backend/schema.graphql"
      via: "GraphQL query matching schema"
      pattern: "users"
    - from: "frontend/src/lib/stores/userSelection.svelte.ts"
      to: "sessionStorage"
...
<task type="auto">
  ...
  <verify>
    1. `pnpm run check` passes (TypeScript/Svelte type checking) from frontend/
...
<task type="auto">
  ...
  <verify>
    1. `pnpm test:run` passes from frontend/ (all tests including new ones)
