...
files_modified:
  ...
  - perspectize-go/internal/core/ports/repositories/user_repository.go
...
must_haves:
  ...
  artifacts:
    - path: "perspectize-go/schema.graphql"
      ...
      contains: "users:"
    - path: "perspectize-fe/src/lib/queries/users.ts"
    ...
    - path: "perspectize-fe/src/lib/queries/content.ts"
      ...
      min_lines: 20
    - path: "perspectize-fe/src/lib/stores/userSelection.svelte.ts"
      provides: "Reactive user selection state with session persistence"
      min_lines: 15
    - path: "perspectize-fe/tests/unit/queries-users.test.ts"
      provides: "Unit tests for user queries"
      min_lines: 15
    - path: "perspective-fe/tests/unit/stores-userSelection.test.ts"
      provides: "Unit tests for user selection store"
      min_lines: 20
  key_links:
    - from: "perspectize-fe/src/lib/queries/users.ts"
      to: "perspectize-go/schema.graphql"
      via: "GraphQL query matching schema"
      pattern: "users"
    - from: "perspectize-fe/src/lib/stores/userSelection.svelte.ts"
      to: "sessionStorage"
      via: "$effect sync with browser guard"
      pattern: "sessionStorage"
    - from: "perspectize-go/internal/adapters/graphql/resolvers/schema.resolvers.go"
      to: "perspectize-go/internal/core/services/user_service.go"
...
@perspectize-go/internal/adapters/repositories/postgres/user_repository.go
@perspectize-go/internal/core/services/user_service.go
@perspectize-go/internal/adapters/graphql/resolvers/resolver.go
@perspectize-go/internal/adapters/graphql/resolvers/schema.resolvers.go
...
<task type="auto">
  ...
  <files>
    perspectize-go/schema.graphql
    perspective-go/internal/core/ports/repositories/user_repository.go
    perspectize-go/internal/adapters/repositories/postgres/user_repository.go
    perspectize-go/internal/core/services/user_service.go
    perspectize-go/internal/adapters/graphql/resolvers/schema.resolvers.go
    perspectize-go/internal/adapters/graphql/generated/generated.go
  ...
  <action>
    ...
    **Step 5 - Code generation:** Run `make graphql-gen` from the `perspectize-go/` directory. This will add the `Users` method signature to the generated `QueryResolver` interface.
    ...
    **Step 7 - Verify:** Run `make test` from `perspectize-go/` to ensure all existing tests still pass (78+ tests). The new `ListAll` method won't have its own test yet — that's fine for v1 scope.
...
<task type="auto">
  ...
  <verify>
    1. `pnpm run check` passes (TypeScript/Svelte type checking) from perspectize-fe/
...
<task type="auto">
  ...
  <verify>
    1. `pnpm test:run` passes from perspectize-fe/ (all tests including new ones)
...
<verification>
1. Backend: `cd perspectize-go && make test` — all tests pass (78+)
2. Backend: `cd perspectize-go && go build ./...` — compiles without errors
3. Frontend: `cd perspectize-fe && pnpm run check` — type checking passes
4. Frontend: `cd perspectize-fe && pnpm test:run` — all tests pass (55+)
