...
must_haves:
  ...
  artifacts:
    - path: "fe/src/routes/+page.svelte"
      ...
      min_lines: 30
    - path: "fe/src/lib/components/ActivityTable.svelte"
      provides: "AG Grid wrapper with content columns, Quick Filter, pagination"
      min_lines: 50
    - path: "fe/src/lib/components/UserSelector.svelte"
    ...
    - path: "fe/src/lib/components/Header.svelte"
      ...
      min_lines: 15
    - path: "fe/tests/components/ActivityTable.test.ts"
...
@fe/src/lib/components/Header.svelte
@fe/src/lib/components/AGGridTest.svelte
...
<task type="auto">
  ...
  <verify>
    1. `pnpm run check` passes from fe/
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    Prerequisites: Start the Go backend (`cd backend && make run`) and the frontend dev server (`cd fe && pnpm run dev`).
...
<verification>
1. Frontend builds: `cd fe && pnpm run build` succeeds
2. Type check: `cd fe && pnpm run check` passes
3. All tests: `cd fe && pnpm test:run` passes
4. Backend: `cd backend && make test` passes
