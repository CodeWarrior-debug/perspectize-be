...
This makes the pagination bar use auto height and wrap controls on narrow viewports instead of overflowing.
  ...
  <verify>
Run `pnpm run check` from frontend/ to ensure no TypeScript errors.
Run `grep -q "min-w-0" perspective-frontend/src/lib/components/Header.svelte` to confirm responsive class added.
Run `grep -q "ag-paging-panel" frontend/src/app.css` to confirm pagination CSS added.
  </verify>
  <done>
...
3. `grep -q "min-w-0" frontend/src/lib/components/Header.svelte && echo "PASS"` — responsive flex fix present
4. `grep -q "ag-paging-panel" frontend/src/app.css && echo "PASS"` — pagination CSS override present
5. `grep -q "shrink-0" frontend/src/lib/components/Header.svelte && echo "PASS"` — interactive element protection present
</verification>
