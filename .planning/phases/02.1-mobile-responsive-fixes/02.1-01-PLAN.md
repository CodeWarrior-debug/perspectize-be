...
This makes the pagination bar use auto height and wrap controls on narrow viewports instead of overflowing.
  ...
  <verify>
Run `pnpm run check` from perspectize-fe/ to ensure no TypeScript errors.
Run `grep -q "min-w-0" perspective-fe/src/lib/components/Header.svelte` to confirm responsive class added.
Run `grep -q "ag-paging-panel" perspectize-fe/src/app.css` to confirm pagination CSS added.
  </verify>
  <done>
...
Use the existing `renderHeader()` helper pattern. For finding the right-side container, query by the button's parent element or by the second div inside the inner container.
  ...
  <verify>
Run `cd perspectize-fe && pnpm run test:run` — all Header tests pass (existing 9 + new 4 = 13 tests).
  </verify>
  <done>
...
<verification>
1. `cd perspectize-fe && pnpm run check` — no TypeScript errors
2. `cd perspectize-fe && pnpm run test:run` — all tests pass including updated Header tests
3. `grep -q "min-w-0" perspectize-fe/src/lib/components/Header.svelte && echo "PASS"` — responsive flex fix present
4. `grep -q "ag-paging-panel" perspectize-fe/src/app.css && echo "PASS"` — pagination CSS override present
5. `grep -q "shrink-0" perspectize-fe/src/lib/components/Header.svelte && echo "PASS"` — interactive element protection present
</verification>
