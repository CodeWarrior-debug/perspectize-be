...
Run `grep -q "updateColumnVisibility" fe/src/lib/components/ActivityTable.svelte && echo "PASS"` — column visibility function present.
Run `grep -q "onGridSizeChanged" fe/src/lib/components/ActivityTable.svelte && echo "PASS"` — grid size handler present.
  </verify>
  <done>
