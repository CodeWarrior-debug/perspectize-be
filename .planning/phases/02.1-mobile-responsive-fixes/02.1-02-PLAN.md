...
4. Add a test: `'column definitions include colId for duration column'` — verify the duration column has a colId for responsive hiding identification.
  ...
  <verify>
Run `cd perspectize-fe && pnpm run test:run` — all ActivityTable tests pass (existing 4 + new 4 = 8 tests).
Run `cd perspectize-fe && pnpm run check` — no TypeScript errors.
Run `grep -q "updateColumnVisibility" perspectize-fe/src/lib/components/ActivityTable.svelte && echo "PASS"` — column visibility function present.
Run `grep -q "onGridSizeChanged" perspectize-fe/src/lib/components/ActivityTable.svelte && echo "PASS"` — grid size handler present.
  </verify>
  <done>
...
- BUG-005: Table columns hide progressively at narrow widths (only Title at 375px)
  ...
  <how-to-verify>
1. Start the frontend dev server: `cd perspectize-fe && pnpm run dev`
2. Start the backend: `cd perspectize-go && make run`
...
2. `cd perspectize-fe && pnpm run test:run` — all tests pass (Header: 13, ActivityTable: 8)
