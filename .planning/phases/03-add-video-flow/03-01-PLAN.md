...
must_haves:
  ...
  artifacts:
    - path: "perspectize-fe/src/lib/utils/youtube.ts"
    ...
    - path: "perspectize-fe/src/lib/components/shadcn/index.ts"
      ...
      contains: "Dialog"
    - path: "perspectize-fe/tests/unit/youtube.test.ts"
      provides: "Unit tests for YouTube URL validation"
      min_lines: 30
  key_links:
    - from: "perspectize-fe/src/lib/queries/content.ts"
      to: "perspectize-go/schema.graphql"
      via: "GraphQL mutation name must match"
      pattern: "createContentFromYouTube"
    - from: "perspectize-fe/src/lib/components/shadcn/index.ts"
      to: "perspectize-fe/src/lib/components/shadcn/dialog/"
...
3. `grep -q "validateYouTubeUrl" perspectize-fe/src/lib/utils/youtube.ts` -- utility exists
4. `grep -q "CREATE_CONTENT_FROM_YOUTUBE" perspectize-fe/src/lib/queries/content.ts` -- mutation exists
5. `grep -q "new URL" perspectize-fe/src/lib/utils/youtube.ts` -- uses URL constructor (not regex, avoids catastrophic backtracking)
6. `grep -q "Dialog" perspectize-fe/src/lib/components/shadcn/index.ts` -- Dialog exported
7. `grep -q "Input" perspectize-fe/src/lib/components/shadcn/index.ts` -- Input exported
8. `grep -q "Label" perspectize-fe/src/lib/components/shadcn/index.ts` -- Label exported
</verification>
