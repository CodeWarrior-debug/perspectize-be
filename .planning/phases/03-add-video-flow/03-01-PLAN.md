...
must_haves:
  ...
  artifacts:
    - path: "fe/src/lib/utils/youtube.ts"
    ...
    - path: "fe/src/lib/components/shadcn/index.ts"
      ...
      contains: "Dialog"
    - path: "fe/tests/unit/youtube.test.ts"
      provides: "Unit tests for YouTube URL validation"
      min_lines: 30
  key_links:
    - from: "fe/src/lib/queries/content.ts"
      to: "backend/schema.graphql"
      via: "GraphQL mutation name must match"
      pattern: "createContentFromYouTube"
    - from: "fe/src/lib/components/shadcn/index.ts"
      to: "fe/src/lib/components/shadcn/dialog/"
...
3. `grep -q "validateYouTubeUrl" fe/src/lib/utils/youtube.ts` -- utility exists
4. `grep -q "CREATE_CONTENT_FROM_YOUTUBE" fe/src/lib/queries/content.ts` -- mutation exists
5. `grep -q "new URL" fe/src/lib/utils/youtube.ts` -- uses URL constructor (not regex, avoids catastrophic backtracking)
6. `grep -q "Dialog" fe/src/lib/components/shadcn/index.ts` -- Dialog exported
7. `grep -q "Input" fe/src/lib/components/shadcn/index.ts` -- Input exported
8. `grep -q "Label" fe/src/lib/components/shadcn/index.ts` -- Label exported
</verification>
