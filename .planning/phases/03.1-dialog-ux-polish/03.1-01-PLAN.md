...
must_haves:
  ...
  artifacts:
    ...
    - path: "fe/static/fonts/Charter-Regular.woff2"
      provides: "Charter Regular 400 font file"
    - path: "fe/static/fonts/Charter-Italic.woff2"
      provides: "Charter Italic 400 font file"
    - path: "fe/static/fonts/Charter-Bold.woff2"
...
@fe/docs/DESIGN_SPEC.md
@fe/src/app.css
@fe/src/lib/components/shadcn/button/button.svelte
@fe/src/lib/components/shadcn/dialog/dialog-content.svelte
</context>
...
Place exactly 3 files in `fe/static/fonts/`:
...
The font files must be placed alongside the existing `Geist-Variable.woff2`.
  ...
  <verify>
Run `ls -la fe/static/fonts/` and confirm all 4 font files exist:
...
Verify woff2 format: `file fe/static/fonts/Charter-*.woff2` should show "Web Open Font Format" or similar binary font indicator (not HTML/text).
  </verify>
  <done>Three Charter woff2 font files exist in fe/static/fonts/ and are non-empty.</done>
</task>

<task type="auto">
  <name>Task 2: Define all design tokens and Charter @font-face in app.css</name>
  <files>fe/src/app.css</files>
  <action>
Replace the existing `@theme` block and add Charter `@font-face` declarations in `fe/src/app.css`. Keep the existing `@import 'tailwindcss'`, Geist `@font-face`, and AG Grid pagination CSS unchanged.
...
- The `--font-family-serif` enables Tailwind's `font-serif` utility to use Charter
  ...
  <verify>
1. Run `pnpm run build` from `fe/` — must succeed with no errors
2. Run `pnpm run check` from `fe/` — must succeed (type-check)
3. Verify the @theme block contains exactly 27 `--color-*` entries: count with `grep -c '\-\-color-' fe/src/app.css` (should be 27)
4. Verify `--font-family-serif` is defined: `grep 'font-family-serif' fe/src/app.css`
5. Verify NO oklch values remain: `grep -c 'oklch' fe/src/app.css` (should be 0)
  </verify>
  <done>
...
<verification>
1. `pnpm run build` succeeds from fe/ — no Tailwind or CSS compilation errors
...
4. `grep -c '\-\-color-' fe/src/app.css` returns 27 (exact token count)
5. `grep -c 'oklch' fe/src/app.css` returns 0 (all hex)
6. `ls fe/static/fonts/Charter-*.woff2 | wc -l` returns 3
7. `grep 'font-family-serif' fe/src/app.css` matches (serif font defined)
</verification>
