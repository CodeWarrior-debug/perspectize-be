...
files_modified:
  ...
  - perspectize-fe/static/fonts/Charter-Regular.woff2
  - perspectize-fe/static/fonts/Charter-Italic.woff2
  - perspective-fe/static/fonts/Charter-Bold.woff2
autonomous: true
...
must_haves:
  ...
  artifacts:
    ...
    - path: "perspective-fe/static/fonts/Charter-Regular.woff2"
      provides: "Charter Regular 400 font file"
    - path: "perspectize-fe/static/fonts/Charter-Italic.woff2"
      provides: "Charter Italic 400 font file"
    - path: "perspectize-fe/static/fonts/Charter-Bold.woff2"
  ...
  key_links:
    ...
    - from: "perspectize-fe/src/app.css"
      to: "perspectize-fe/static/fonts/"
...
@perspectize-fe/docs/DESIGN_SPEC.md
@perspectize-fe/src/app.css
@perspective-fe/src/lib/components/shadcn/button/button.svelte
@perspectize-fe/src/lib/components/shadcn/dialog/dialog-content.svelte
</context>
...
<task type="auto">
  ...
  <files>
    perspectize-fe/static/fonts/Charter-Regular.woff2
    perspectize-fe/static/fonts/Charter-Italic.woff2
    perspectize-fe/static/fonts/Charter-Bold.woff2
  </files>
  <action>
...
Place exactly 3 files in `perspectize-fe/static/fonts/`:
...
The font files must be placed alongside the existing `Geist-Variable.woff2`.
  ...
  <verify>
Run `ls -la perspectize-fe/static/fonts/` and confirm all 4 font files exist:
...
Each Charter file should be non-empty (at least a few KB).
  ...
  <done>Three Charter woff2 font files exist in perspectize-fe/static/fonts/ and are non-empty.</done>
</task>

<task type="auto">
  <name>Task 2: Define all design tokens and Charter @font-face in app.css</name>
  <files>perspectize-fe/src/app.css</files>
  <action>
Replace the existing `@theme` block and add Charter `@font-face` declarations in `perspectize-fe/src/app.css`. Keep the existing `@import 'tailwindcss'`, Geist `@font-face`, and AG Grid pagination CSS unchanged.
...
- Do NOT add `--opacity-disabled` to the `@theme` block (disabled is handled via `disabled:opacity-50` utility class already in shadcn button)
...
- The `--font-family-serif` enables Tailwind's `font-serif` utility to use Charter
  ...
  <verify>
1. Run `pnpm run build` from `perspectize-fe/` — must succeed with no errors
2. Run `pnpm run check` from `perspectize-fe/` — must succeed (type-check)
3. Verify the @theme block contains exactly 27 `--color-*` entries: count with `grep -c '\-\-color-' perspectize-fe/src/app.css` (should be 27)
4. Verify `--font-family-serif` is defined: `grep 'font-family-serif' perspectize-fe/src/app.css`
5. Verify NO oklch values remain: `grep -c 'oklch' perspectize-fe/src/app.css` (should be 0)
  </verify>
  <done>
...
<verification>
1. `pnpm run build` succeeds from perspectize-fe/ — no Tailwind or CSS compilation errors
...
4. `grep -c '\-\-color-' perspectize-fe/src/app.css` returns 27 (exact token count)
5. `grep -c 'oklch' perspectize-fe/src/app.css` returns 0 (all hex)
6. `ls perspectize-fe/static/fonts/Charter-*.woff2 | wc -l` returns 3
7. `grep 'font-family-serif' perspective-fe/src/app.css` matches (serif font defined)
</verification>
