...
<task type="auto">
  ...
  <files>fe/src/lib/components/ActivityTable.svelte</files>
  <action>
Update the `themeQuartz.withParams()` call in `fe/src/lib/components/ActivityTable.svelte` to apply the design system colors. The current configuration is:
...
- The AG Grid themeQuartz API may not support all of these parameters — check the AG Grid v32 theming docs. If a parameter is not supported, omit it (the grid will use its defaults). The critical parameters are: fontFamily, headerBackgroundColor, headerTextColor, oddRowBackgroundColor, evenRowBackgroundColor, borderColor, accentColor.
  ...
  <verify>
1. Run `pnpm run build` from `fe/` — must succeed
2. Run `pnpm run check` from `fe/` — must succeed
3. Run `pnpm run test:run` from `fe/` — all tests pass (ActivityTable tests still pass)
4. Verify the theme uses Geist: `grep "Geist" fe/src/lib/components/ActivityTable.svelte`
5. Verify navy header color: `grep "1a365d" fe/src/lib/components/ActivityTable.svelte`
  </verify>
  <done>
