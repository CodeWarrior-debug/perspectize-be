...
- `perspectize-go/schema.graphql` - Added channelTitle, publishedAt, tags, description fields to Content type; extended ContentSortBy with VIEW_COUNT, LIKE_COUNT, PUBLISHED_AT; added search to ContentFilter
- `perspectize-go/internal/core/domain/pagination.go` - Added ContentSortByViewCount, ContentSortByLikeCount, ContentSortByPublishedAt constants; added Search field to ContentFilter
- `perspectize-go/internal/adapters/graphql/model/models_gen.go` - Generated GraphQL models with new fields (via make graphql-gen)
- `perspectize-go/internal/adapters/graphql/generated/generated.go` - Updated GraphQL resolvers (via make graphql-gen)
- `perspectize-go/internal/adapters/graphql/resolvers/helpers.go` - Extract channelTitle, publishedAt, tags, description from YouTube response JSON with nil checks
- `perspectize-go/internal/adapters/repositories/postgres/content_repository.go` - JSONB extraction for VIEW_COUNT/LIKE_COUNT/PUBLISHED_AT sort; ILIKE search on name; NULLS LAST for JSONB sorts
- `perspectize-go/internal/adapters/graphql/resolvers/schema.resolvers.go` - Map search filter from GraphQL to domain
- `perspectize-go/internal/adapters/youtube/client.go` - Updated YouTubeAPIResponse struct with tags and publishedAt fields
