...
- `fe/src/lib/components/AddVideoPopover.svelte` — Added `window.dispatchEvent(new CustomEvent('content-added'))` in onSuccess callback
- `fe/src/lib/components/ActivityTable.svelte` — Added `$effect` listener for `content-added` event that resets pagination and refetches
