...
- `frontend/src/lib/components/AddVideoPopover.svelte` — Added `window.dispatchEvent(new CustomEvent('content-added'))` in onSuccess callback
- `frontend/src/lib/components/ActivityTable.svelte` — Added `$effect` listener for `content-added` event that resets pagination and refetches
