...
must_haves:
  truths:
    ...
    - "fe/ folder no longer exists; fe/ folder contains all frontend code"
    - "CI workflow working-directory and cache paths reference backend/ and fe/"
    - "Zero references to backend/ or fe/ remain in source-controlled files"
...
<objective>
Rename the monorepo folders (`backend/` -> `backend/`, `fe/` -> `fe/`) and update ALL path references across CI/CD workflows, CLAUDE.md files, documentation, and planning artifacts.
...
<task type="auto">
  ...
  <action>
    ...
    **Step 3: Update CI workflow - frontend-test.yml**
    In `.github/workflows/frontend-test.yml`, replace ALL occurrences of `fe` with `fe`:
    - `paths: - 'fe/**'` -> `paths: - 'fe/**'` (appears twice: pull_request and push)
    ...
    - `path: fe/coverage/` -> `path: fe/coverage/`
  ...
  <verify>
    ...
    4. `ls fe/ 2>/dev/null` returns error (old folder gone)
...
<task type="auto">
  ...
  <action>
    **Step 1: Update root CLAUDE.md**
    Replace `backend` with `backend` and `fe` with `fe` throughout. Be careful with the `backend/CLAUDE.md` reference -- it becomes `backend/CLAUDE.md`. Same for `perspective-fe/CLAUDE.md` -> `fe/CLAUDE.md`.
    ...
    ```bash
    sed -i '' -e 's/backend/backend/g' -e 's/fe/fe/g' CLAUDE.md
    ...
    **Step 3: Update frontend CLAUDE.md**
    ```bash
    sed -i '' -e 's/fe/fe/g' fe/CLAUDE.md
    sed -i '' -e 's/backend/backend/g' fe/CLAUDE.md
    sed -i '' 's/perspectize/perspectize/g' fe/CLAUDE.md
    ```

    **Step 4: Update docs/ directory (8 markdown files)**
    ```bash
    find docs/ -name "*.md" -type f -exec sed -i '' -e 's/backend/backend/g' -e 's/fe/fe/g' {} +
    find docs/ -name "*.md" -type f -exec sed -i '' 's/perspectize/perspectize/g' {} +
    ```

    **Step 5: Update .planning/ directory**
    ```bash
    find .planning/ -name "*.md" -type f -exec sed -i '' -e 's/backend/backend/g' -e 's/fe/fe/g' {} +
    ...
    **Step 7: Check for any remaining references**
    ...
    grep -r "fe/" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=fe
    grep -r "perspectize" . --exclude-dir=node_modules --exclude-dir=.git
    ```
    Fix any remaining references found. Expected: zero results for backend/ and fe/. The `perspectize` search may find references in git history descriptions within planning summaries -- those are historical records and acceptable.
    ...
    **CAUTION with sed replacements:**
    ...
    - Order matters: run the folder name replacements FIRST (`backend` -> `backend`, `fe` -> `fe`), THEN the repo name replacement (`perspectize` -> `perspectize`). This prevents double-replacement issues.
  ...
  <verify>
    ...
    2. `grep -r "fe/" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=fe` returns zero results (except package.json "name" field which is acceptable)
  ...
  <done>
    - All CLAUDE.md files reference backend/ and fe/ (not backend/ or fe/)
...
2. fe/ renamed to fe/ (git mv preserves history)
