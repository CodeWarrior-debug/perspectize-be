...
user_setup:
  - service: digitalocean-app-platform
    why: "Backend deployment source directory must be updated from backend/ to backend/"
    dashboard_config:
...
<task type="checkpoint:human-action" gate="blocking">
  ...
  <action>
    ...
    from `backend/` to `backend/`. This requires access to the DigitalOcean dashboard.
  </action>
  <how-to-do-it>
    ...
    4. Change Source Directory from `backend/` to `backend/`
...
<task type="auto">
  ...
  <action>
    ...
    **Step 3: Verify backend API is accessible**
    ...
    ```bash
    curl -s -X POST https://backend-dz3qa.ondigitalocean.app/graphql \
      -H "Content-Type: application/json" \
      -d '{"query": "{ __typename }"}' | grep -q "Query" && echo "Backend API OK" || echo "Backend API FAIL"
    ```

    Note: The backend URL (`backend-dz3qa.ondigitalocean.app`) does NOT change when the source directory changes -- it's a DigitalOcean-assigned subdomain.

    **Step 4: Final stale reference check**
    Run one last sweep to confirm no old paths leaked through:
    ```bash
    grep -rn "backend/" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=backend --include="*.go" --include="*.yml" --include="*.yaml" --include="*.md" --include="*.json" --include="*.ts" --include="*.svelte"
    grep -rn "frontend/" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=fe --include="*.go" --include="*.yml" --include="*.yaml" --include="*.md" --include="*.json" --include="*.ts" --include="*.svelte"
    ```
...
- Completeness: No references to backend/, frontend/, or perspectize in source files
</verification>
