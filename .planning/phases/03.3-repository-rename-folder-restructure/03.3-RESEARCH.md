...
Phase 3.3 involves renaming the GitHub repository from `perspectize-be` to `perspectize` and restructuring the monorepo folder layout (`perspectize-go/` → `backend/`, `perspectize-fe/` → `fe/`). This is primarily an infrastructure refactoring with significant impacts on Go import paths, CI/CD configurations, documentation, and deployment configurations.

The repository currently has:
- **40 Go files** with the module path `github.com/CodeWarrior-debug/perspectize-be/perspectize-go`
- **78 files** containing import statements with the old module path
- **117 occurrences** of `perspectize-be` across 52 files
- **65 files** referencing `perspectize-go/` path
- **58 files** referencing `perspectize-fe/` path
...
### Current Structure
```
perspectize-be/                          # GitHub repo name
├── perspectize-go/                       # Backend folder
│   ├── go.mod                           # module github.com/.../perspectize-be/perspectize-go
│   ├── internal/
│   │   ├── core/domain/*.go             # Import: github.com/.../perspectize-be/perspectize-go/internal/core/domain
│   │   └── ...
│   └── pkg/
├── perspectize-fe/                       # Frontend folder
│   ├── package.json                     # name: "perspectize-fe"
│   └── ...
├── .github/workflows/
│   ├── ci.yml                           # working-directory: perspectize-go
│   └── frontend-test.yml                # cache-dependency-path: perspectize-fe/pnpm-lock.yaml
├── docs/*.md                            # References to perspectize-go/, perspectize-fe/
...
├── backend/                              # Renamed from perspectize-go/
│   ├── go.mod                           # module github.com/.../perspectize/backend
│   ├── internal/
│   │   ├── core/domain/*.go             # Import: github.com/.../perspectize/backend/internal/core/domain
│   │   └── ...
│   └── pkg/
├── fe/                                   # Renamed from perspectize-fe/
│   ├── package.json                     # name: "perspectize-fe" (unchanged, npm package name)
│   └── ...
├── .github/workflows/
│   ├── ci.yml                           # working-directory: backend
│   └── frontend-test.yml                # cache-dependency-path: fe/pnpm-lock.yaml
├── docs/*.md                            # Updated to backend/, fe/
...
- `perspectize-go/` folder still exists (matches working directory expectations)
...
**What:** Rename `perspectize-go/` → `backend/` and `perspectize-fe/` → `fe/`, then update all references across configs, docs, and planning files
...
sed -i '' 's/perspectize-go/backend/g' .github/workflows/ci.yml

# .github/workflows/frontend-test.yml
sed -i '' 's/perspectize-fe/fe/g' .github/workflows/frontend-test.yml
```

**3.3: Update root CLAUDE.md**
```bash
sed -i '' 's/perspectize-go/backend/g' CLAUDE.md
sed -i '' 's/perspectize-fe/fe/g' CLAUDE.md
```

**3.4: Update package-level CLAUDE.md files**
```bash
sed -i '' 's/perspectize-go/backend/g' backend/CLAUDE.md
sed -i '' 's/perspectize-fe/fe/g' fe/CLAUDE.md
```

**3.5: Update docs/ directory (8 markdown files)**
```bash
find docs/ -name "*.md" -type f -exec sed -i '' 's/perspectize-go/backend/g' {} +
find docs/ -name "*.md" -type f -exec sed -i '' 's/perspectize-fe/fe/g' {} +
```

**3.6: Update .planning/ directory (69 markdown files)**
```bash
find .planning/ -name "*.md" -type f -exec sed -i '' 's/perspectize-go/backend/g' {} +
find .planning/ -name "*.md" -type f -exec sed -i '' 's/perspectize-fe/fe/g' {} +
```
...
# Update if needed
sed -i '' 's/perspectize-go/backend/g' .gitignore
sed -i '' 's/perspectize-fe/fe/g' .gitignore
```

**3.8: Update any remaining references**
```bash
# Search for remaining references
grep -r "perspectize-go" . --exclude-dir=node_modules --exclude-dir=.git
grep -r "perspectize-fe" . --exclude-dir=node_modules --exclude-dir=.git
...
- **Backend URL:** `https://perspectize-go-dz3qa.ondigitalocean.app`
- **Backend Source Directory:** `perspectize-go/` (configured in App Platform)
- **Frontend:** To be deployed with Source Directory `perspectize-fe/`

**Required updates:**

**Backend service (via DigitalOcean App Platform UI or API):**
1. Navigate to App Platform → perspectize backend service → Settings
2. Update **Source Directory** from `perspectize-go/` → `backend/`
3. Verify **Build Command** still works (should be auto-detected)
4. Save changes

**Frontend static site (when deployed):**
1. Set **Source Directory** to `fe/` (not `perspectize-fe/`)
...
services:
  - name: backend
    source_dir: backend/  # Updated from perspectize-go/
    # ... rest of config

static_sites:
  - name: frontend
    source_dir: fe/       # Updated from perspectize-fe/
    # ... rest of config
```
...
- Backend URL remains `https://perspectize-go-dz3qa.ondigitalocean.app` (URL doesn't change with source directory)
...
**Why it happens:** `gqlgen.yml` contains full import paths like `github.com/.../perspectize-be/perspectize-go/internal/core/domain.SortOrder`. These aren't caught by searching for `*.go` files.
...
- Error: "package github.com/.../perspectize-be/perspectize-go/internal/core/domain is not in GOROOT"
...
```yaml
cache-dependency-path: perspectize-go/go.sum  # Old path, no longer exists
```
...
**What goes wrong:** Future work references old paths ("`perspectize-go/internal/...`"), causing confusion during implementation.
...
**What goes wrong:** Pushing folder rename to `main` triggers auto-deployment, but App Platform still points to old Source Directory `perspectize-go/`. Deployment fails with "Source directory not found".
...
**Warning signs:**
- Deployment logs: "Error: Source directory 'perspectize-go' not found"
- App Platform status: "Deployment failed"
- Backend URL returns 500 or 503 errors

### Pitfall 5: Package.json Name Confusion

**What goes wrong:** Renaming `perspectize-fe/` folder to `fe/`, developer updates `package.json` name to `"fe"`, breaking npm package conventions.
...
**How to avoid:** **Do NOT update `package.json` name field.** Leave as `"perspectize-fe"` even after folder rename:
```json
{
  "name": "perspectize-fe",  // KEEP THIS - npm package name
  "private": true,
  // ...
}
```
...
**What goes wrong:** After rename, developers search for `perspectize-go` in codebase to find backend code, get zero results, assume feature isn't implemented.
...
3. Use descriptive folder names: `backend/` is self-documenting, `perspectize-go/` required context
...
- [ ] Navigate to https://github.com/CodeWarrior-debug/perspectize-be/settings
- [ ] Rename repository from `perspectize-be` → `perspectize`
...
- [ ] Update `perspectize-go/go.mod`: `go mod edit -module github.com/CodeWarrior-debug/perspectize/backend`
- [ ] Bulk update Go imports: `find perspectize-go/ -name "*.go" -type f -exec sed -i '' 's|github.com/CodeWarrior-debug/perspectize-be/perspectize-go|github.com/CodeWarrior-debug/perspectize/backend|g' {} +`
- [ ] Update gqlgen.yml: `sed -i '' 's|github.com/CodeWarrior-debug/perspectize-be/perspectize-go|github.com/CodeWarrior-debug/perspectize/backend|g' perspectize-go/gqlgen.yml`
- [ ] Verify: `cd perspectize-go && go mod tidy` (no errors)
- [ ] Verify: `cd perspectize-go && go test ./...` (all tests pass)
- [ ] Verify: `cd perspectize-go && make graphql-gen` (regenerates cleanly)
- [ ] Commit: `git commit -am "refactor(go): update module path to perspectize/backend"`

### Stage 3: Folder Rename
- [ ] Rename folders: `git mv perspectize-go backend && git mv perspectize-fe fe`
- [ ] Update CI workflows: `sed -i '' 's/perspectize-go/backend/g' .github/workflows/ci.yml`
- [ ] Update CI workflows: `sed -i '' 's/perspectize-fe/fe/g' .github/workflows/frontend-test.yml`
- [ ] Update root CLAUDE.md: `sed -i '' -e 's/perspectize-go/backend/g' -e 's/perspectize-fe/fe/g' CLAUDE.md`
- [ ] Update backend CLAUDE.md: `sed -i '' 's/perspectize-go/backend/g' backend/CLAUDE.md`
- [ ] Update frontend CLAUDE.md: `sed -i '' 's/perspectize-fe/fe/g' fe/CLAUDE.md`
- [ ] Update docs/: `find docs/ -name "*.md" -type f -exec sed -i '' -e 's/perspectize-go/backend/g' -e 's/perspectize-fe/fe/g' {} +`
- [ ] Update .planning/: `find .planning/ -name "*.md" -type f -exec sed -i '' -e 's/perspectize-go/backend/g' -e 's/perspectize-fe/fe/g' {} +`
- [ ] Update .gitignore (if needed): Check `grep "perspectize-go\|perspectize-fe" .gitignore`
- [ ] Verify: `grep -r "perspectize-go/" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=backend` (no results)
- [ ] Verify: `grep -r "perspectize-fe/" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=fe` (no results)
- [ ] Verify: `cd backend && go test ./...` (tests still pass from new location)
- [ ] Verify: `cd fe && pnpm run build` (build succeeds from new location)
- [ ] Commit: `git commit -am "refactor: rename folders to backend/ and fe/"`

### Stage 4: Deployment Configuration Update
- [ ] Navigate to DigitalOcean App Platform → Backend Service → Settings
- [ ] Update Source Directory: `perspectize-go/` → `backend/`
...
- [ ] Verify: Backend URL `https://perspectize-go-dz3qa.ondigitalocean.app/graphql` still responds
...
sed -i '' 's|github.com/CodeWarrior-debug/perspectize-be/perspectize-go|github.com/CodeWarrior-debug/perspectize/backend|g' gqlgen.yml
...
echo "Checking for remaining 'perspectize-go/' references..."
BACKEND_REFS=$(grep -r "perspectize-go/" . \
  --exclude-dir=node_modules \
  --exclude-dir=.git \
  --exclude-dir=backend \
  --exclude="*.md" | wc -l)

echo "Checking for remaining 'perspectize-fe/' references..."
FRONTEND_REFS=$(grep -r "perspectize-fe/" . \
  --exclude-dir=node_modules \
  --exclude-dir=.git \
  --exclude-dir=fe \
  --exclude="*.md" | wc -l)
...
2. **Custom domain for backend**
   - What we know: Backend URL is `perspectize-go-dz3qa.ondigitalocean.app` (auto-generated subdomain)
...
- Repository grep analysis - 78 files with Go import paths, 117 occurrences of `perspectize-be`, confirmed via local search
