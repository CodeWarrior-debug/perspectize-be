...
Phase 3.3 involves renaming the GitHub repository from `perspectize` to `perspectize` and restructuring the monorepo folder layout (`backend/` → `backend/`, `fe/` → `fe/`). This is primarily an infrastructure refactoring with significant impacts on Go import paths, CI/CD configurations, documentation, and deployment configurations.
...
├── docs/*.md                            # References to backend/, fe/
...
**What:** Rename `backend/` → `backend/` and `fe/` → `fe/`, then update all references across configs, docs, and planning files
...
- **Frontend:** To be deployed with Source Directory `fe/`
...
1. Set **Source Directory** to `fe/` (not `fe/`)
...
**What goes wrong:** Renaming `fe/` folder to `fe/`, developer updates `package.json` name to `"fe"`, breaking npm package conventions.
...
- [ ] Update CI workflows: `sed -i '' 's/fe/fe/g' .github/workflows/frontend-test.yml`
- [ ] Update root CLAUDE.md: `sed -i '' -e 's/backend/backend/g' -e 's/fe/fe/g' CLAUDE.md`
- [ ] Update backend CLAUDE.md: `sed -i '' 's/backend/backend/g' backend/CLAUDE.md`
- [ ] Update frontend CLAUDE.md: `sed -i '' 's/fe/fe/g' fe/CLAUDE.md`
- [ ] Update docs/: `find docs/ -name "*.md" -type f -exec sed -i '' -e 's/backend/backend/g' -e 's/fe/fe/g' {} +`
- [ ] Update .planning/: `find .planning/ -name "*.md" -type f -exec sed -i '' -e 's/backend/backend/g' -e 's/fe/fe/g' {} +`
...
- [ ] Verify: `grep -r "fe/" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=fe` (no results)
...
echo "Checking for remaining 'fe/' references..."
FRONTEND_REFS=$(grep -r "fe/" . \
  --exclude-dir=node_modules \
  --exclude-dir=.git \
  --exclude-dir=fe \
  --exclude="*.md" | wc -l)
