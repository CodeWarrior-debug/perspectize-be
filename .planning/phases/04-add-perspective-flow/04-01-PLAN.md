...
files_modified:
  ...
  - frontend/src/lib/components/shadcn/progress/
  - frontend/src/lib/components/shadcn/textarea/
  - frontend/src/lib/components/shadcn/index.ts
  - frontend/src/lib/components/RatingInput.svelte
  - frontend/src/lib/components/VideoSelector.svelte
  - frontend/tests/unit/queries-perspectives.test.ts
  - frontend/tests/components/RatingInput.test.ts
  - frontend/tests/components/VideoSelector.test.ts
autonomous: true
...
must_haves:
  truths:
    ...
    - "pnpm run test:coverage exits 0 (all thresholds pass: 80% stmts/lines/functions, 75% branches)"
  artifacts:
    ...
    - path: "frontend/src/lib/components/RatingInput.svelte"
      ...
      min_lines: 40
    - path: "frontend/src/lib/components/VideoSelector.svelte"
    ...
    - path: "frontend/src/lib/components/shadcn/index.ts"
      ...
      contains: "Progress"
    - path: "frontend/tests/unit/queries-perspectives.test.ts"
      provides: "Unit tests for perspective query definitions"
      min_lines: 20
    - path: "frontend/tests/components/RatingInput.test.ts"
  ...
  key_links:
    ...
    - from: "frontend/src/lib/components/RatingInput.svelte"
      to: "frontend/src/lib/components/shadcn/index.ts"
      via: "imports Progress component from barrel"
      pattern: "Progress"
    - from: "frontend/src/lib/components/VideoSelector.svelte"
...
<verification>
1. `cd fe && pnpm run test:coverage` -- exits 0, all coverage thresholds pass (80% stmts/lines/functions, 75% branches)
2. `pnpm run check` -- no TypeScript errors
3. `grep -q "Progress" frontend/src/lib/components/shadcn/index.ts` -- Progress exported
4. `grep -q "Textarea" frontend/src/lib/components/shadcn/index.ts` -- Textarea exported
5. `grep -q "createPerspective" frontend/src/lib/queries/perspectives.ts` -- mutation defined
6. `grep -q "CREATE_PERSPECTIVE" frontend/src/lib/queries/perspectives.ts` -- mutation exported
7. `test -f frontend/src/lib/components/RatingInput.svelte` -- RatingInput exists
8. `test -f frontend/src/lib/components/VideoSelector.svelte` -- VideoSelector exists
</verification>
...
- Coverage thresholds maintained (pnpm run test:coverage exits 0)
- Type-check clean
</success_criteria>
