...
must_haves:
  ...
  artifacts:
    - path: "frontend/tests/components/UserSelector.test.ts"
      provides: "UserSelector component tests covering render, selection, query states"
      min_lines: 40
    - path: "perspective-frontend/tests/components/ActivityTable.test.ts"
      provides: "ActivityTable tests covering grid rendering, search, pagination handlers"
      min_lines: 60
    - path: "frontend/tests/unit/stores-userSelection.test.ts"
      provides: "userSelection store tests covering setUser, clearUser, session sync"
      min_lines: 30
  key_links:
    - from: "frontend/tests/components/UserSelector.test.ts"
      to: "frontend/src/lib/components/UserSelector.svelte"
      via: "imports and renders component"
      pattern: "import.*UserSelector"
    - from: "frontend/tests/components/ActivityTable.test.ts"
...
@perspective-frontend/src/lib/stores/userSelection.svelte.ts
</context>
...
<task type="auto">
  ...
  <files>frontend/coverage/ (generated output, not committed)</files>
  <action>
Run `cd fe && pnpm run test:coverage` to get the current coverage report. This will likely fail due to thresholds not met — that is expected. Examine the text output to identify exact uncovered line ranges for each source file. Focus on:
...
frontend/tests/components/UserSelector.test.ts
frontend/tests/components/ActivityTable.test.ts
perspective-frontend/tests/unit/stores-userSelection.test.ts
  </files>
  <action>
...
After writing tests, run `cd fe && pnpm run test:coverage`. If any threshold is still not met, identify remaining gaps and add more tests. Iterate until ALL thresholds pass:
- Lines: >= 80%
- Functions: >= 80%
- Branches: >= 75%
- Statements: >= 80%
  </action>
  <verify>Run `cd fe && pnpm run test:coverage`. Command must exit 0 with all thresholds met. All tests must pass (no failures).</verify>
...
<verification>
1. `cd fe && pnpm run test:coverage` exits 0 (all thresholds pass)
2. `cd fe && pnpm run test:run` shows zero test failures
