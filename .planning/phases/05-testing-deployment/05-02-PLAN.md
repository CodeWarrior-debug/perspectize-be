...
files_modified:
  ...
  - frontend/static/.nojekyll (DELETE)
  - .github/workflows/frontend-deploy.yml (DELETE)
autonomous: false

must_haves:
  truths:
    - "Frontend is deployed and accessible at a public Sevalla URL (https://fe-xxxxx.sevalla.app)"
    - "Pushing to main auto-deploys the frontend via Sevalla (not GitHub Actions)"
...
<objective>
Clean up GitHub Pages artifacts and deploy the SvelteKit frontend to Sevalla as a static site.

Purpose: The frontend needs to be publicly accessible. The backend is already on Sevalla (backend-dz3qa.sevalla.app), so deploying the frontend on the same platform keeps infrastructure unified. The previous GitHub Pages setup left artifacts (base path, .nojekyll, deploy workflow) that must be removed first.

Output: Frontend deployed at a public `*.sevalla.app` URL, GitHub Pages artifacts removed, test CI workflow preserved. The assigned frontend URL will be captured for use in Plan 05-03 (CORS configuration).
</objective>
...
.github/workflows/frontend-deploy.yml (DELETE)
  ...
  <action>
**1. Update `frontend/svelte.config.js`:**
...
Change to unconditionally empty base path (Sevalla serves from root):
```javascript
...
- Sevalla serves from root domain, not a subpath

**2. Delete `frontend/static/.nojekyll`:**

This is a GitHub Pages artifact to bypass Jekyll processing. Not needed on Sevalla.

```bash
rm frontend/static/.nojekyll
```

**3. Delete `.github/workflows/frontend-deploy.yml`:**

This is the GitHub Pages deployment workflow. Sevalla handles deployment via its own GitHub integration (auto-deploy on push to main).
...
```
  ...
  <verify>
1. `grep "base: ''" frontend/svelte.config.js` finds the empty base path
2. `grep -c "perspectize" frontend/svelte.config.js` returns 0 (no more GitHub Pages base path)
3. `test ! -f perspective-frontend/static/.nojekyll && echo "deleted"` outputs "deleted"
...
<task type="checkpoint:human-action" gate="blocking">
  <action>Deploy the frontend as a static site on Sevalla</action>
  <instructions>
**Before you start:** The code changes from Task 1 must be committed and pushed to `main` before Sevalla can build from it. If still on a feature branch, merge to main first.

**Deploy the frontend on Sevalla:**

1. Go to https://app.sevalla.com/apps — click "Create App"
...
5. **Environment variables** (CRITICAL - set scope to BUILD_TIME):
   ...
   - Value: `https://backend-dz3qa.sevalla.app/graphql`
   - Scope: **BUILD_TIME** (NOT RUN_TIME - static sites have no runtime)

6. **App name:** `frontend` (or similar)
...
11. Open DevTools Network tab. You should see a GraphQL request to `backend-dz3qa.sevalla.app/graphql`. It will likely fail with a CORS error — this is expected and will be fixed in Plan 05-03.

12. **Report back with:**
    - The assigned frontend URL (e.g., `https://fe-xxxxx.sevalla.app`)
    - Whether the site loaded (HTML/CSS renders)
    - Whether GraphQL requests show CORS errors (expected at this stage)
  </instructions>
  <resume-signal>Provide the assigned Sevalla frontend URL and confirm the site loads. Example: "deployed at https://fe-xxxxx.sevalla.app, site loads, GraphQL CORS error as expected"</resume-signal>
</task>

</tasks>

<verification>
1. `frontend/svelte.config.js` has `base: ''` (no subpath)
2. `frontend/static/.nojekyll` does NOT exist
...
7. Frontend is accessible at the assigned Sevalla URL (confirmed by user)
</verification>
...
- Frontend is deployed on Sevalla as a static site
...
IMPORTANT: The summary MUST include the assigned Sevalla frontend URL in the "Key Results" section. Plan 05-03 depends on this URL for CORS configuration.
</output>
