...
files_modified:
  ...
  - perspectize-fe/static/.nojekyll (DELETE)
  - .github/workflows/frontend-deploy.yml (DELETE)
autonomous: false

must_haves:
  truths:
    - "Frontend is deployed and accessible at a public DigitalOcean App Platform URL (https://perspectize-fe-xxxxx.ondigitalocean.app)"
    ...
    - "SvelteKit base path is empty (root hosting, no /perspectize-be subpath)"
  ...
  key_links:
    - from: "perspectize-fe/src/lib/queries/client.ts"
...
Purpose: The frontend needs to be publicly accessible. The backend is already on DigitalOcean App Platform (perspectize-go-dz3qa.ondigitalocean.app), so deploying the frontend on the same platform keeps infrastructure unified. The previous GitHub Pages setup left artifacts (base path, .nojekyll, deploy workflow) that must be removed first.
...
```bash
cd perspectize-fe && pnpm run build
```
...
```
  ...
  <verify>
1. `grep "base: ''" perspectize-fe/svelte.config.js` finds the empty base path
2. `grep -c "perspectize-be" perspectize-fe/svelte.config.js` returns 0 (no more GitHub Pages base path)
...
6. `cd perspectize-fe && pnpm run build` succeeds
7. `cd perspectize-fe && pnpm run test:coverage` exits 0, coverage not regressed
  </verify>
...
2. **Source configuration:**
   - Source: GitHub
   - Repository: `CodeWarrior-debug/perspectize-be`
   - Branch: `main`
...
6. **App name:** `perspectize-fe` (or similar)
...
11. Open DevTools Network tab. You should see a GraphQL request to `perspectize-go-dz3qa.ondigitalocean.app/graphql`. It will likely fail with a CORS error — this is expected and will be fixed in Plan 05-03.

12. **Report back with:**
    - The assigned frontend URL (e.g., `https://perspectize-fe-xxxxx.ondigitalocean.app`)
    - Whether the site loaded (HTML/CSS renders)
    - Whether GraphQL requests show CORS errors (expected at this stage)
  </instructions>
  <resume-signal>Provide the assigned DigitalOcean frontend URL and confirm the site loads. Example: "deployed at https://perspectize-fe-xxxxx.ondigitalocean.app, site loads, GraphQL CORS error as expected"</resume-signal>
</task>

</tasks>

<verification>
1. `perspectize-fe/svelte.config.js` has `base: ''` (no subpath)
2. `perspectize-fe/static/.nojekyll` does NOT exist
3. `.github/workflows/frontend-deploy.yml` does NOT exist
4. `.github/workflows/frontend-test.yml` exists and is valid YAML
5. `cd perspectize-fe && pnpm run build` succeeds
6. `cd perspectize-fe && pnpm run test:coverage` exits 0
