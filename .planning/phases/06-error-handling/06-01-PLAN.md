...
must_haves:
  truths:
    ...
    - "CreateFromYouTube returns the existing content item on duplicate URL (idempotent, no error) — calling CreateFromYouTube twice with same URL returns identical content.ID on both calls (no error on second call)"
...
// After:
existing, err := s.repo.GetByURL(ctx, url, userID)
if err == nil && existing != nil {
    return existing, nil
}
```
...
**File:** `backend/test/services/content_service_test.go`

Update tests to expect success on duplicate URL instead of `ErrAlreadyExists`. Find test(s) that assert error on duplicate URL and change to assert:
- Success (no error)
- Returned content ID matches existing content ID
...
All tests must pass with the new idempotent behavior.
