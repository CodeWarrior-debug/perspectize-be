...
<task type="auto">
  ...
  <action>
    ...
    2. Rename old sqlx repository files to preserve them as reference:
       ...
       ```
       Move these functions/types there:
       ...
       - `JSONBArray` type (custom type for jsonb[] columns — used by PerspectiveModel.CategorizedRatings in gorm_models.go)
       - `CategorizedRatingArray` type (if distinct from JSONBArray — check perspective_repository.go)
