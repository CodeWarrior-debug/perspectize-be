...
must_haves:
  truths:
    ...
    - "Unit tests exist for useCreateUser hook, CreateUserPopover component, and CREATE_USER mutation"
    - "All tests pass and coverage thresholds met (80% stmts/lines/functions, 75% branches)"
  artifacts:
    ...
    - path: "frontend/tests/unit/hooks-useCreateUser.test.ts"
      provides: "Unit tests for useCreateUser mutation hook"
    - path: "frontend/tests/components/CreateUserPopover.test.ts"
      provides: "Component tests for CreateUserPopover"
  key_links:
...
<task type="auto">
  <name>Task 6: Unit tests for new components and hooks</name>
  <files>
    frontend/tests/unit/queries-users.test.ts
    frontend/tests/unit/hooks-useCreateUser.test.ts
    frontend/tests/components/CreateUserPopover.test.ts
  ...
  <action>
    **queries-users.test.ts** — Extend existing test file:
    - CREATE_USER is a mutation operation
    - CREATE_USER contains createUser field
    - CreateUserInput requires username, email optional

    **hooks-useCreateUser.test.ts** — New test file (mirror useAddVideo test patterns):
    - useCreateUser returns a mutation object
    - Mutation function calls graphqlClient.request with correct params
    - Verify query key structure matches queryKeys.users.list()

    **CreateUserPopover.test.ts** — New component test:
    - Renders trigger button with UserPlus icon
    - Username input field exists in popover content
    - Submit disabled when username is empty
    - Calls onUserCreated callback with new user ID on success

    **UserSelector.test.ts** — Update existing tests:
    - Verify CreateUserPopover is rendered adjacent to select
    - Auto-selects new user after creation (setSelectedUserId called)

    **AddVideoPopover.test.ts** — Update existing tests:
    - Verify FormPopover is used (no behavior regression)
    - Existing test assertions still pass

    Follow existing patterns: mock graphqlClient, mock TanStack Query hooks, use vitest + @testing-library/svelte.
  </action>
  <verify>
    - `cd frontend && pnpm run test:run` all tests pass
    - `cd frontend && pnpm run test:coverage` — no coverage regression (80% stmts/lines/functions, 75% branches)
  </verify>
  <done>Unit tests written for all new code. CREATE_USER mutation tested, useCreateUser hook tested, CreateUserPopover component tested, UserSelector updated tests pass, AddVideoPopover refactor tested. Coverage thresholds met.</done>
</task>
